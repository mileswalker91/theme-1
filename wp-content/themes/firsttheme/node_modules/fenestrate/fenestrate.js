...
function spawn(cmd, workingDir, cb) {
  if (windows) cmd = "cmd /c " + cmd;
  cmd = cmd.split(' ');
  childProcess.spawn(cmd[0], cmd.slice(1), { cwd: workingDir, stdio: 'inherit'}).on('close', function(code) {
    cb(null, code);
  })
}
...
function reinstallNodeModules(p, prod, cb) {
  rimraf(path.resolve(p, "./node_modules"), function(err) {
    ...
    if (err) {
      ...
      spawn(cmd, p, function() {
        spawn('npm dedupe', p, cb);
      });
    }
  });
}
